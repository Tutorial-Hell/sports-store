...
When using Neon MCP tools, always use these identifiers for the sportstore database:

- `projectId`: `round-hill-73876833`
- `org_id`: `org-raspy-water-74666637` (Vercel: Tutorial-Hell's projects)
- Default branch: `main` (development branch)
- The Neon project is named `neon-cyan-plank` (auto-generated by Vercel)
- Auth tables live in the `neon_auth` schema; app tables live in the `public` schema
- The MCP authenticates as `proteus77@protonmail.com` — this account has access via the org above
